...
== Important Config Files and Settings

. inventories/group_vars/all/manifest.yaml
.. Set `launcher:` to `true` in order activate installation
.. Update version numbers where necessary

. inventories/group_vars/all/common.yml
.. Set `eval_seed_users_count:` to desired user count

. roles/msbroker/tasks/main.yml
.. Set launcher URLs

. roles/rhsso/tasks/main.yml
.. Get RHSSO secure route

. roles/rhsso/tasks/apply_ocp_roles.yaml
.. Set Fuse Managed service account privileges

. playbooks/generate-customisation-inventory.yml
.. Set RH SSO Playbook vars

== Hosts File Format

. Note your final host file must in this format:
+
[source]
----
[local:vars]
...
run_master_tasks=false eval_app_host=apps.GUID.open.redhat.com
openshift_master_url=https://master.GUID.open.redhat.com
